...
`opencode.ci.json` is a standalone OpenCode CI configuration template. It declares the `plainconcepts` provider at `http://172.30.0.30:10000` with apiKey `awf-openai-proxy`, registers `glm-5-2` ("GLM 5.2") and `glm-5-1` ("GLM 5.1") models, defaults to `plainconcepts/glm-5-2`, and includes the `ci-workflow-agent` agent definition with output discipline directive. LSP is disabled for csharp, fsharp, and razor; consumers not working with .NET can remove the `lsp` block. JSON (RFC 8259) does not permit comments, so the ownership header is documented in a companion `opencode.ci.json.md` file. Workflow templates install to `.github/workflows/`; `opencode.ci.json` installs to the repository root.
